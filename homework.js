...
if (require.main === module) {
  async function runTests() {
    ...
    console.log('\n--- 任務三：Promise 整合 ---');
    try {
      const data = await getProductsAndCart();
      if (data && data.products && data.cart) {
        console.log('getProductsAndCart: 成功');
        console.log(
          `  產品 ${data.products.length} 筆，購物車 ${data.cart.carts?.length || 0} 筆`,
        );
      } else {
        console.log('getProductsAndCart: 回傳格式不正確');
      }
    } catch (error) {
      console.log('getProductsAndCart 錯誤:', error.message);
    }

    // 任務四測試
    console.log('\n--- 任務四：管理員 API ---');
    if (ADMIN_TOKEN === 'your-admin-token') {
      console.log('請先設定你的 ADMIN_TOKEN！');
    } else {
      try {
        const orders = await getOrders();
        console.log(
          'getOrders:',
          orders ? `成功取得 ${orders.length} 筆訂單` : '回傳 undefined',
        );
      } catch (error) {
        console.log('getOrders 錯誤:', error.message);
      }
    }
